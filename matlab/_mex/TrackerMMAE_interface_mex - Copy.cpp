...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if (!strcmp("getStatePrediction", cmd))
	{
		...
		VectorXd statePred = Tracker_instance->getStatePrediction();
	...
	if (!strcmp("getStateCovariancePrediction", cmd))
	{
		...
		MatrixXd covMat = Tracker_instance->getStateCovariancePrediction();
	...
	{
		VectorXd statePost = Tracker_instance->getStatePosterior();
	...
	if (!strcmp("getStateCovariancePosterior", cmd))
	{
		...
		MatrixXd covMat = Tracker_instance->getStateCovariancePosterior();
		plhs[0] = eigenMatrixToMatlab(covMat);
	...
	if (!strcmp("getAllModelProbabilities", cmd))
	{
		...
		std::vector<double> probs = Tracker_instance->getAllModelProbabilities();
