...
{	
    ...
    int median_window = *mxGetPr(prhs[1]);
    int numberOfFramesBeforeDestruction = *mxGetPr(prhs[2]);
    int numberOfFramesBeforeDestructionLocked = *mxGetPr(prhs[3]);
    double creation_threshold = *mxGetPr(prhs[4]);
    double validation_gate = *mxGetPr(prhs[5]);
    double metric_weight = *mxGetPr(prhs[6]);
    double recognition_threshold = *mxGetPr(prhs[7]);
    double c_learning_rate = *mxGetPr(prhs[8]);
    double const_pos_var = *mxGetPr(prhs[9]);
    double const_vel_var = *mxGetPr(prhs[10]);
    double const_accel_var = *mxGetPr(prhs[11]);
    
        plhs[0] = convertPtr2Mat<PersonList>(new PersonList(median_window, numberOfFramesBeforeDestruction, numberOfFramesBeforeDestructionLocked, creation_threshold, validation_gate, metric_weight, recognition_threshold, c_learning_rate, const_pos_var, const_vel_var, const_accel_var));
        return;
    }
    ...
    if (!strcmp("delete", cmd)) {
        ...
        destroyObject<PersonList>(prhs[1]);
    ...
    PersonList *PersonList_instance = convertMat2Ptr<PersonList>(prhs[1]);
    ...
    if(!strcmp("updateDeltaT", cmd))
	{
	  ...
	  PersonList_instance->updateDeltaT(deltaT);
	  return;
	}
